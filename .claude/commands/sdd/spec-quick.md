...
allowed-tools: Read, SlashCommand, TodoWrite, Bash, Write, Glob, mcp__atlassian__getJiraIssue, mcp__atlassian__getAccessibleAtlassianResources
argument-hint: <project-description-or-jira-url> [--auto]
---
...
- Check if description is a Jira ticket:
...
"NAME-7463 --auto" → mode=automatic, jiraKey="NAME-7463"
"https://inspira.atlassian.net/browse/PROJ-123" → mode=interactive, jiraKey="PROJ-123"
```
...
1. **Detect Input Type and Fetch Jira (if applicable)**:
   - Check if description contains Jira ticket identifier:
     - Jira URL pattern: `https://[domain].atlassian.net/browse/[KEY-123]`
     - Jira key pattern: `[A-Z]+-[0-9]+`
   - If Jira detected:
     a. Extract Jira key from URL or use key directly
     b. Use `mcp__atlassian__getAccessibleAtlassianResources` to get cloudId
     c. Use `mcp__atlassian__getJiraIssue` to fetch ticket
     d. Extract fields:
     - **summary** → Use for feature name generation
     - **description** → Primary project description
     - **Acceptance Criteria** → Extract from description (look for "Acceptance Criteria", "AC:", "Success Criteria")
     - **issuelinks** → Note linked/blocked-by tickets
       e. Construct enriched description:
     ...
     ## Acceptance Criteria
     {Extracted AC or "See Jira ticket"}

     ## Related Tickets
     {Linked tickets if any}
     ```

     f. Store jiraKey for placeholder replacement

   - If Jira fetch fails, log warning and use description as-is
   - If no Jira detected, use description as-is, set jiraKey to null

2. **Generate Feature Name**:
   - Convert description (or Jira summary) to kebab-case
...
5. **Initialize Files from Templates**:
   ...
   {{PROJECT_DESCRIPTION}} → enriched description (from Jira or original input)
   {{JIRA_TICKET}} → jiraKey if detected (e.g., "NAME-7463"), or null
   ```
...
7. **Output Progress**:
   ...
   [If from Jira: 📋 Source: JIRA-KEY (link)]
   ```
...
- Jira integration is automatic (no flags required)
- If Jira fetch fails, fall back gracefully to plain description
- Use regex `[A-Z]{2,}-\d+` to detect Jira keys
...
- **mcp**atlassian**getAccessibleAtlassianResources**: Get cloudId for Jira API
- **mcp**atlassian**getJiraIssue**: Fetch Jira ticket details when key/URL detected
